		...
		if($key){
			...
			if(is_array($validators) && !empty($validators)){
				foreach($validators as $vname){
					...
					if($vname === 'number'){
						if(!is_numeric($value)){
							$err_msgs[] = '<strong>'. $field_label .'</strong> '. THWCFE_i18n::t('is not a valid number.');
						}
					}else{
						...
						if(is_array($validator)){
							...
							if(preg_match($pattern, $value) === 0) {
								...
								$err_msgs[] = sprintf(THWCFE_i18n::t($validator['message']), $flabel);
							}
						}else{
							...
							if(is_array($cnf_validator)){
								...
								if($value && $cvalue && $value != $cvalue) {
									$err_msgs[] = sprintf(THWCFE_i18n::t($cnf_validator['message']), $flabel );
								}
							}
						}
					}
	...
	****************************************/
	...
	public function prepare_ajax_conditions_data_section($section){
		...
		if($section->get_property('conditional_rules_ajax_json')){
			...
			$data_str = 'id="'.$section->name.'" data-rules="'. stripslashes($rules) .'" data-rules-action="'. $rules_action .'" data-rules-elm="section"';
		}
		return $data_str;
	}
	...
	public function woo_form_field_heading($field, $key, $args, $value){
		...
		$data_rules = 'data-rules="'.stripslashes($rules).'" data-rules-action="'.$rules_action.'"';
	...
	public function form_field_wrapper_attributes($attributes, $key, $args){
		...
		if(isset($args['rules']) && !empty($args['rules'])){
			...
			$attributes[] = 'data-rules="'. stripslashes($rules) .'"';
    ...
    public function woo_form_field($ofield, $key, $args, $value = null ) {
        ...
        $field_container = '<p class="form-row %1$s" id="%2$s" data-priority="' . esc_attr( $sort ) . '" data-rules="'. stripslashes($rules) .'" data-rules-action="'.$rules_action.'" data-validations="'.$validations_str.'">%3$s</p>';
	...
	public function woo_form_field_hidden($field, $key, $args, $value){
		...
		$field .= 'class="'.esc_attr(implode(' ', $css_class)).'" '.$price_info.' data-rules="'. stripslashes($rules) .'" data-rules-action="'.$rules_action.'" />';
		return $field;
	}
